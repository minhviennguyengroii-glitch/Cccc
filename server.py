...
def confirm_bypass():
    ...
    if not is_admin_token:
        # CHECK 3: Bypass detection — token phải được tạo ít nhất 5 giây trước
  ...
  .then(function(d){
    ...
    if(d.exists){
      ...
      box.innerHTML='<div class="result-box"><div class="result-title"><i class="fa-solid fa-circle-info" style="color:var(--primary);"></i> Thông tin Key</div>'
        ...
        +'<div class="info-row"><span class="info-label">Thời hạn</span><span class="info-val">'+fmt(d.duration)+'</span></div>'
        +'<div class="info-row"><span class="info-label">Hết hạn lúc</span><span class="info-val">'+fmt(d.han_dung)+'</span></div>'
        +'<div class="info-row"><span class="info-label">Thiết bị</span><span class="info-val">'+fmt(usedc)+' / '+fmt(maxd)+'</span></div>'
        +'<div class="info-row"><span class="info-label">Kích hoạt lúc</span><span class="info-val">'+fmt(d.activated_time_str)+'</span></div>'
        +'<div class="info-row"><span class="info-label">Ngày tạo</span><span class="info-val">'+fmt(d.created_at_str)+'</span></div>'
        +'</div>';
    } else {
...
function renderDbTable(){
  ...
  var rows=dbAll.filter(function(k){
    return !f||(k.key||'').toLowerCase().includes(f)||(k.creator_info||'').toLowerCase().includes(f)||(k.key_status||'').toLowerCase().includes(f);
  });
  _dbKeyArr=rows.map(function(k){return k.key;});
  var html='';
  rows.forEach(function(k,i){
    var st=k.key_status||k.status||'—';
    var valid=k.is_valid||false;
    var badge=valid?'<span class="badge badge-yes">Hợp lệ</span>':(st==='Chưa kích hoạt'?'<span class="badge badge-warn">Chưa KH</span>':'<span class="badge badge-no">Hết hạn</span>');
    var typStr=(k.key||'').startsWith('FREE')?'FREE':((k.key||'').startsWith('VIP')?'VIP':'KEY');
    var devUsed=typeof k.used_devices==='number'?k.used_devices:(Object.keys(k.used_devices||{}).length||0);
    html+='<tr>'
      +'<td><span class="key-val" onclick="copyText(_dbKeyArr['+i+'])" style="cursor:pointer;font-size:0.68rem;" title="Click để copy">'+k.key+'</span></td>'
      +'<td><span class="badge badge-warn">'+typStr+'</span></td>'
      +'<td style="font-size:0.73rem;color:var(--text2);">'+fmt(k.han_dung||k.expiry_date||'—')+'</td>'
      +'<td style="text-align:center;">'+fmt(devUsed)+' / '+fmt(k.max_devices||'—')+'</td>'
      +'<td>'+badge+'</td>'
